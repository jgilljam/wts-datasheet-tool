"""Datenblatt-Tool: PDF / Stichworte / Anfrage → WTS-Komponente + DE/EN-PDFs.

Eingangsfunktion: render() — wird vom App-Entry oder als Page in st.navigation eingehängt.
"""
...
from core.config import ACCENT, ANTHRACITE, TEXT_SECONDARY, gemini_settings
from lib.generator import render_pdf_bytes
from lib.library_search import find_best_match, load_library
...
def render() -> None:
    """Komplette Datenblatt-Tool-Page rendern."""
    render_header(
        "Datenblatt-Tool",
        "PDF oder Stichworte → WTS-Komponente + Datenblätter (DE/EN)",
    )
                    ...
                    if sel.startswith("wiz::lib::"):
                        ...
                        if t.datenblatt_url and t.datenblatt_url.lower().startswith(("http://", "https://")):
                            ...
                            try:
                                ...
                                text = extract_text_from_uploaded(io.BytesIO(pdf_bytes))
